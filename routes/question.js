...
var ITEM_QUERY = [
    "SELECT",
        "QUESTION_NBR_CDE AS id, ",
        "QUESTION_LEVEL_CDE AS grade, ",
        "QUESTION_TYP_CDE AS type, ",
        "QUESTION_SEQ_NO AS seqNo, ",
        "QUESTION_ENGL_CONTS AS header, ",
        "INSTRUCT_ENGL_CONTS AS choices",
    "FROM",
        "ESLS_STUDY_QUESTION",
    "WHERE",
        "QUESTION_NBR_CDE = ?"
...
router.get('/:id', function(req, res) {
  ...
  db_pool.acquire(function(err, db) {
    db.query(ITEM_QUERY.join(' '), [questionId], function(err, rows, columns) {
      db_pool.release(db);
